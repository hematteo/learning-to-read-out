...
class Crosscoder(
    ...
    def init_encoder_with_decoder_transpose(self, factor: float = 1.0):
        # DTensor-safe path: do the rearrange on the local shard, then wrap
        # back as DTensor with W_E's placements before copy_. The naive
        # einops.rearrange(self.W_D, ...) path on a DTensor was observed to
        # deadlock at 100% GPU util with no log progress, likely because the
        # reshape/permute composition under DTensor triggers an unexpected
        # collective. W_D and W_E share Shard(0) on n_heads, and we permute
        # only the trailing two dims, so the operation is local per shard.
        if isinstance(self.W_D, DTensor):
            W_D_local = self.W_D.to_local()
            transposed_local = (
                einops.rearrange(W_D_local, "n_heads d_sae d_model -> n_heads d_model d_sae").clone().contiguous()
                * factor
            )
            transposed_dtensor = DTensor.from_local(
                transposed_local,
                device_mesh=self.W_E.device_mesh,
                placements=self.W_E.placements,
            )
            self.W_E.copy_(transposed_dtensor)
        else:
            transposed_decoder = (
                einops.rearrange(self.W_D, "n_heads d_sae d_model -> n_heads d_model d_sae").clone().contiguous()
                * factor
            )
