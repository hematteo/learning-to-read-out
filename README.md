...
models, train a crosscoder over that trajectory, and use the resulting
dictionaries to track how a sparse output readout forms, reorganizes, and becomes
load-bearing during pretraining. This is the code release for the paper
**Learning to Read Out: Unembedding Dynamics in Language Model Pretraining**
(also thesis **Chapter 4**).

> **Came from a figure in Chapter 4?** Open [`docs/REPRODUCE.md`](docs/REPRODUCE.md),
> find your figure label in the index, and follow it to the experiment and the
> script that produces its metrics. Want to *reuse* the method instead? See
...
and thesis figures are rendered in the separate thesis LaTeX tree from metrics
...
the thesis figures it backs and the metric files that underlie them (the metrics
are regenerated by running the scripts, not shipped; figures are rendered in the
thesis LaTeX tree, not in this repo).
